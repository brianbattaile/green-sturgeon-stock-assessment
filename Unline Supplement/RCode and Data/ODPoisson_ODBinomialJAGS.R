...
Count2022S= load_data(Count2022S)
...
# detection (hyper) prior
      #Changed to mu to correspond to paper
     mu~dnorm(1,0.1) #dnorm(0,1) #Eq.18
# Abundance prior
     alpha.lam~dunif(0,3) #Eq.14
# Abundance noise
     for (i in 1:n.sites){
...
params<-c("totalN", "mean.abundance", "mean.N", "alpha.lam", "mean.eps", "sd.lam","tau.lam",
          "mu", "tau.p", "mean.detection", "sd.p","fit", "fit.new", "lp",
...
#plot(out.jags.BpoisOD,plot.type=c("histogram", "trace"), vars="mu",)

#Plot S1-S2 using 2021 data
MCMC<-combine.mcmc(out.jags.BpoisOD$mcmc) #combine.mcmc from runjags package
fit_vector_mu<-data.frame(MCMC) %>%
  select(starts_with("mu")) %>% 
  as.matrix() %>% 
  c() %>%
  unlist()

fit_vector_sd.p<-data.frame(MCMC) %>%
  select(starts_with("sd.p")) %>% 
  as.matrix() %>% 
  c() %>%
  unlist()

fit_vector_lp<-data.frame(MCMC) %>%
...
MU<-data.frame(fit_vector_mu)
Mu = MU %>% ggplot(aes(fit_vector_mu)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
Mu + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

SD.P<-data.frame(fit_vector_mu)
Sd.p = SD.P %>% ggplot(aes(fit_vector_sd.p)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
Sd.p + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

LP<-data.frame(fit_vector_lp)
Lp = LP %>% ggplot(aes(fit_vector_lp)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
Lp + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

PP<-data.frame(fit_vector_p)
Pp = PP %>% ggplot(aes(fit_vector_p)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
Pp + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

rmu<-rnorm(10000,1, 0.1) #rnorm(10000,1,0.1)
rMU<-data.frame(rmu)
rMu = rMU %>% ggplot(aes(rmu)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
rMu + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

rsd.p<-runif(10000,0,1) #runif(10000,0,1)
rSD.P<-data.frame(rsd.p)
rSd.p = rSD.P %>% ggplot(aes(rsd.p)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
rSd.p + theme(plot.title = element_text(hjust = 0.5))+
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

rdelta<-rnorm(10000,rmu,rsd.p)
rDELTA<-data.frame(rdelta)
rDelta = rDELTA %>% ggplot(aes(rdelta)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
rDelta + theme(plot.title = element_text(hjust = 0.5))+
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())

rp<-exp(rdelta)/(1 + exp(rdelta))
rPP<-data.frame(rp)
rPp = rPP %>% ggplot(aes(rp)) +
  geom_histogram(position="identity",bins=50, fill="white",color="gray") +
  ggtitle("") +
  xlab("") + 
  ylab("") +
  theme_classic()
rPp + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=20)) +
  theme(axis.text.y=element_blank()) +
  theme(axis.ticks.y=element_blank())
