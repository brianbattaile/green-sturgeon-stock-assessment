...
#This model runs 2021 data but is a bad GofFit to all years and 2021 estimates are off
jags.model.string <- "
model {
  # Priors
  #Detection prior
  ...
  disp ~ dunif(0,20)
  NBvar<-1/disp
  NBmean <- NBvar / (NBvar + lambda) # overdispersion
...
params<-c("totalN", "mean.N", "alpha.p", "beta.p", "mean.p", "p.derived", "rho.derived", "lambda", "NBmean", "NBvar", "disp", "omega", "zinf", "fit", "fit.new", "N", "Nz", "z", "p") 
...
# run jags
out.jags.BBZINB <- run.jags(model = jags.model.string,
                     data = jags.data,
                     monitor = params,
                     inits = inits,
                     n.chains = nc,
...
# Print summary
SumStatBBZINB<-as.data.frame(round(summary(out.jags.BBZINB), 3)[ , c(1:5, 9, 11)]) 
SumStatBBZINB
...
FIT <- map(seq(1:nc), ~unlist(out.jags$mcmc[.x][,"fit"])) %>% 
  unlist()
FITNEW <- map(seq(1:nc), ~unlist(out.jags$mcmc[.x][,"fit.new"])) %>% 
  unlist() 
