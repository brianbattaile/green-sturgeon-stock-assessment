...
#THis model did not work for 2020 data....didn't try for 2021-2022
jags.model.string <- "
model {
  # Priors
  zinf ~ dbeta(1,1)
...
params<-c("totalN", "mean.N", "alpha.p", "beta.p", "omega", "zinf", "mean.p", "p.derived", "rho.derived", "lambda", "fit", "fit.new", "N", "p", "z") 
...
# run jags
out.jags.BBZIP <- run.jags(model = jags.model.string,
                     data = jags.data,
                     monitor = params,
                     inits = inits,
                     n.chains = nc,
...
# Print summary
SumStatBBZIP<-as.data.frame(round(summary(out.jags.BBZIP), 3)[ , c(1:5, 9, 11)]) 
SumStatBBZIP
