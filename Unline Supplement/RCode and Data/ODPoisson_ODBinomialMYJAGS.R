...
for(k in 1:n.years)
{
alpha.lam[k]~dnorm(0,1) #Eq.14 dnorm(0,3)
mu[k]~dunif(0,1) #Eq.18
}
...
for (i in 1:n.sites)
{
  for (k in 1:n.years)
{
  N[i,k] ~ dpois(lambda[i,k]) #Eq.9 with added subscript k for census 
  log(lambda[i,k])<-alpha.lam[k]+eps[i] #Eq.10 with added subscript k for census
  # detection component
   for (j in 1:n.surveys)
  {
    Y[i,j,k] ~ dbin(p[i,j,k], N[i,k]) #Eq.15 with added subscript k for census
    p[i,j,k]<-exp(lp[i,j,k])/(1+exp(lp[i,j,k])) #Eq.16 logit back transform with added subscript k for census
    lp[i,j,k]~dnorm(mu[k], tau.p) #Eq.17 with added subscript k for census, See v95i02 for linear model example
    
    #Assess model fit using Chi-squared discrepeancy
    #Compute fit statistic E for observed data
    eval[i,j,k]<-p[i,j,k]*N[i,k] #Eq.22 with added subscript k for census
    E[i,j,k]<-pow((Y[i,j,k]-eval[i,j,k]),2)/(eval[i,j,k]+0.5) #Eq.23 with added subscript k for census
    
    #Generate replicate data and compute fit stats for them
    y.new[i,j,k]~dbin(p[i,j,k],N[i,k]) #Eq.24 with added subscript k for census
    E.new[i,j,k]<-pow((y.new[i,j,k]-eval[i,j,k]),2)/(eval[i,j,k]+0.5) #Eq.25 with added subscript k for census
  } # close j loop
  ik.p[i,k]<-mean(p[i,,k]) 
} # Close the k loop
} # close i loop
...
params<-c("totalN","alpha.lam", "sd.lam", "mu",
...
#plot(out.jags.BODPoisMY,plot.type=c("histogram", "trace"), vars="mu",)
